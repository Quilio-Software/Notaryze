...
#include "SpecialStringProcessing.h"
...
bool AdvancedTableComponent::checkFileTypeIsValid (juce::File file)
{
    std::vector<juce::String> allowedFileTypes = {".wav", ".mp3", ".aiff" };
...
bool AdvancedTableComponent::isInterestedInFileDrag (const juce::StringArray& files)
{
    for (auto file : files)
    {
        if (checkFileTypeIsValid (juce::File (file)))
        {
            return true;
        }
    }

/*
  ==============================================================================
    ...
    KeychainManager.h
    Created: 11 Jul 2023 4:15:24pm
    Author:  Abhishek Shivakumar

  ==============================================================================
*/

//TODO: This flaming sack of shit has no iOS support. So do not pretend that this is a generic keyChain management solution.
//TODO: We are going to rewrite this from the ground up if we have to.
...
class Credentials
{
    bool updateEntry (const UsernameAndPassword& creds)
    {
    ...
    bool userCredentialsExist (const Username& username)
    {
        const auto allEntriesForUser = getAllAvailableEntries (username);
        return ! allEntriesForUser.isEmpty ();
    }
    
    StringArray getAllAvailableEntries (const String& filter)
    {
        JUCE_AUTORELEASEPOOL
        {
            StringArray entries;
    ...
    String getPasswordForUsername (const Username& username)
    {
        JUCE_AUTORELEASEPOOL
        {
            const auto serviceName = [[NSString alloc] initWithUTF8String: ProjectInfo::projectName];
    ...
    Array<UsernameAndPassword> getAllStoredUsernamesAndPasswords (std::function<bool ()> onNoneFound)
    {
    ...
    bool removeCredential(const Username& username)
    {
        jassertfalse; //tbd
    }
}
...
public:
    ...
    String getAppIdFromSignature (const File& f)
    {
        String appId;
        JUCE_AUTORELEASEPOOL
        {
    ...
    String getSignerIdentity (const File& f)
    {
...
//TODO: Add the Yum Credential header, please
